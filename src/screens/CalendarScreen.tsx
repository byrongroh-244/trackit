...
function DayPanel({ dateKey, items, allAssignments, onNavigate }: {
  ...
  return (
    <div style={{
      ...
      {isEmpty && (
        <div style={{ padding: '14px 16px', fontSize: 13, color: Colors.textHint }}>
      ...
      {/* Assignments due */}
      {active.map((a, i) => {
        const d2 = daysUntil(a.dueDate);
        const u2 = getUrgencyConfig(d2);
        return (
          <div
            key={a.id}
            ...
            style={{
              ...
              padding: '11px 16px',
              borderBottom: i < active.length - 1 || done.length > 0 || stepsToday.length > 0
                ? '0.5px solid #E3EBEA' : 'none',
              cursor: 'pointer',
          ...
          >
            {/* Subject color bar */}
            <div style={{ width: 3, height: 36, borderRadius: 2, background: u2.accent, flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: Colors.textPrimary, letterSpacing: '-0.01em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {a.name}
              </div>
              <div style={{ fontSize: 12, color: Colors.textHint, marginTop: 2, display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: a.classColor, display: 'inline-block' }} />
                {a.className}
                {isTestOrQuiz(a.type) && (
            ...
            <IconArrow size={15} color={Colors.textHint} />
          </div>
        );
      })}
      ...
      {/* Completed */}
      {done.map(a => (
        <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', opacity: 0.4 }}>
          <div style={{ width: 3, height: 28, borderRadius: 2, background: Colors.textHint, flexShrink: 0 }} />
          <div style={{ fontSize: 13, color: Colors.textSecondary, textDecoration: 'line-through', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}>
            {a.name}
          </div>
        </div>
      ))}
      ...
      {stepsToday.length > 0 && (
        <>
          <div style={{ padding: '8px 16px 4px', fontSize: 10, fontWeight: 600, color: Colors.textHint, textTransform: 'uppercase', letterSpacing: '0.06em', borderTop: active.length > 0 || done.length > 0 ? '0.5px solid #E3EBEA' : 'none' }}>
            Steps due
          </div>
          {stepsToday.map(({ step, assignment: a }, i) => (
            <div
              key={step.id}
              onClick={() => onNavigate('detail', a.id)}
              style={{
                ...
                padding: '10px 16px',
                borderBottom: i < stepsToday.length - 1 ? '0.5px solid #E3EBEA' : 'none',
                cursor: 'pointer', transition: 'background 0.12s',
              }}
              onMouseEnter={e => (e.currentTarget.style.background = Colors.background)}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
            >
              <div style={{ width: 14, height: 14, borderRadius: '50%', border: `1.5px solid ${a.classColor}`, flexShrink: 0, marginTop: 2 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, color: Colors.textPrimary, lineHeight: 1.4 }}>{step.text}</div>
                <div style={{ fontSize: 11, color: Colors.textHint, marginTop: 2, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: a.classColor, display: 'inline-block' }} />
                  {a.name}
                </div>
              </div>
              <IconArrow size={14} color={Colors.textHint} />
            </div>
          ))}
        </>
      )}
